...
def get_a_tform_b(frame_tree_snapshot, frame_a, frame_b, validate=True):
    ...
    def _accumulate_transforms(parent_edges):
        ret = MathHelpers.SE3Pose.from_identity()
        for parent_edge in parent_edges:
            ret = MathHelpers.SE3Pose.from_proto(parent_edge.parent_tform_child) * ret
        return ret
...
def express_se2_velocity_in_new_frame(frame_tree_snapshot, frame_b, frame_c, vel_of_a_in_b,
    ...
    vel_of_a_in_c = MathHelpers.transform_se2velocity(c_adjoint_b, vel_of_a_in_b)
...
def express_se3_velocity_in_new_frame(frame_tree_snapshot, frame_b, frame_c, vel_of_a_in_b,
    ...
    vel_of_a_in_c = MathHelpers.transform_se3velocity(c_adjoint_b, vel_of_a_in_b)
    return vel_of_a_in_c
